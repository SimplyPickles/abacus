use crate::{AbacusError, Value, units::interval::EvalResult};
...
impl FunctionOp {
    pub fn scalar(name: &'static str, min_args: usize, max_args: usize, f: fn(&[Value]) -> Result<Value, AbacusError>) -> Self {
        Self {
            name,
            min_args,
            max_args,
    ...
    pub fn eval_result(name: &'static str, min_args: usize, max_args: usize, f: fn(&[Value]) -> Result<EvalResult, AbacusError>) -> Self {
        Self {
            name,
            min_args,
            max_args,
